...
pub mod client {
    ...
    use crate::xai_api::documents_client::DocumentsClient;
    ...
    ///
    /// # Returns
    /// * `Result<DocumentsClient<InterceptedService<Channel, ClientInterceptor>>, Error>` - The connected client or connection error
    ///
    pub async fn new(
        api_key: &str,
    ) -> Result<DocumentsClient<InterceptedService<Channel, ClientInterceptor>>, Error> {
    ...
    ///
    /// # Returns
    /// * `DocumentsClient<InterceptedService<Channel, ClientInterceptor>>` - The connected client
    pub fn with_channel(
        channel: Channel,
        api_key: &str,
    ) -> DocumentsClient<InterceptedService<Channel, ClientInterceptor>> {
        let auth_intercept = common::interceptor::auth(api_key);
        let client = DocumentsClient::with_interceptor(channel, auth_intercept);
    ...
    ///
    /// # Returns
    /// * `Result<DocumentsClient<InterceptedService<Channel, ClientInterceptor>>, tonic::transport::Error>`
    ...
    pub async fn with_interceptor(
        ...
        let channel = common::channel::new().await?;
        let client = DocumentsClient::with_interceptor(channel, ClientInterceptor::new(interceptor));
        Ok(client)
    }
    ...
    ///
    /// # Returns
    /// * `DocumentsClient<InterceptedService<Channel, ClientInterceptor>>` - The intercepted client
    pub fn with_channel_and_interceptor(
        channel: Channel,
        interceptor: impl Interceptor + Send + Sync + 'static,
    ) -> DocumentsClient<InterceptedService<Channel, ClientInterceptor>> {
        DocumentsClient::with_interceptor(channel, ClientInterceptor::new(interceptor))
    }
}
